...
# For simplicity we will NOT test defaults that are changed by th user only parameters from I-IV are verified 
...
external_databases_path="/mnt/md0/external_databases/"
results_dir="./results"
...
show_all_parameters() {
    ...
    echo ""
    echo "  --projectDir PATH               Directory with projects repository"
    echo ""
    echo "  --external_databases_path PATH  Directory with all databases used by the program"
    echo ""
    echo "  --main_image VALUE              Name of the docker image with main program"
    echo ""
    echo "  --manta_image VALUE             Name of the docker image with manta program"
    echo ""
    echo "  --medaka_image VALUE            Name of the docker image with medaka program"
    echo ""
    echo "  --alphafold_image VALUE         Name of the docker image with alphafold program"
    echo ""
...
if [[ "$machine" == "Illumina" ]]; then
	...
	[[ -z "${max_depth}" ]] && max_depth=600
        ...
        [[ -z "${pval}" ]] && pval=0.05
	[[ -z "${first_round_pval}" ]] && pval=0.25
	[[ -z "${second_round_pval}" ]] && pval=0.05
        [[ -z "${lower_ambig}" ]] && lower_ambig=0.45
        [[ -z "${upper_ambig}" ]] && upper_ambig=0.55
        [[ -z "${window_size}" ]] && window_size=50
